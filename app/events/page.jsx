  ...
  return (
    <section className='w-full flex-wrap flex items-center justify-center my-auto px-6 md:px-4 gap-12 py-8'>
      {eventData.map(data => {
        return <>
          <div key={data.id}
  class="flex flex-col rounded-lg bg-white shadow-md md:max-w-md md:flex-row ">
    <div className=''>
      <img
        loading='lazy'
        class="md:max-w-[16rem] h-80 w-full object-cover md:h-72"
        src={data.src}
        alt="" />
    </div>
  <div class="relative flex flex-col justify-start p-6">
    <h5
